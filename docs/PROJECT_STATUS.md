...
### 5.20. `.python-version` ép cứng "đúng 3.11" → `uv run` cảnh báo trên máy chỉ có 3.12+
...
### 5.21. `api/webui/` (gitignored) — OneDrive/thư mục cũ có thể giữ lại bản build cũ
...
- **`start.bat` build lại WebUI theo 2 tín hiệu** (gộp khi merge `7099623`): lệch commit git so với
  `api/webui/.build-commit` (§5.21) **hoặc** file `webui/src` mới hơn bản build (sửa chưa commit).
